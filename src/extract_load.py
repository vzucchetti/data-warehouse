...
def find_commodities_data(
    symbol, period="5d", interval="1d"
):  # parameters for the function; getting the one value of ticker for the last 5 days
    ticker = yf.Ticker(symbol)  # access the data from Yahoo Finance
    data = ticker.history(period=period, interval=interval)[
        ["Close"]
    ]  # return a dataframe with the close price of stock
    data["symbol"] = symbol
    return data
